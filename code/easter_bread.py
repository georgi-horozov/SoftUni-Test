...
while price_loaf <= budget: # ДОКАТО ЦЕНАТА Е ПО-МАЛКА ОТ БЮДЖЕТА
    loaf_counter += 1 # БРОЯЧ НА КОЗУНАЦИТЕ, КОЙТО СЕ УВЕЛИЧАВА С 1
    budget -= price_loaf
    coloured_count_eggs += 3 # БРОЯТ НА ЦВЕТНИТЕ ЯЙЦА, КОЙТО СЕ УВЕЛИЧАВА С 3
    if loaf_counter % 3 == 0: # УСЛОВИ, ПРИ КОЕТО ГУБИМ ЦВЕТНИ ЯЙЦА
        coloured_count_eggs -= (loaf_counter - 2)

print(f"You made {loaf_counter} loaves of Easter bread! Now you have {coloured_count_eggs} eggs and {budget:.2f}BGN left.")
